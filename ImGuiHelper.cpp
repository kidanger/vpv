...
namespace ImGui {
    ...
    void PlotMultiEx(
            ...
            float scale_min,
            float scale_max,
            ImVec2 graph_size, int stride, float max)
    {
        ...
        if (IsHovered(inner_bb, 0))
        {
            ...
            else if (plot_type == ImGuiPlotType_Histogram)
            {
                for (int dataIdx = 0; dataIdx < num_datas; ++dataIdx)
                {
                    const float v0 = getter(datas,dataIdx, idx0, stride);
                    if(max > -1) {
                        TextColored(colors[dataIdx], "%.1f: %8.4g | %s", (float)(v_idx*max)/stride, v0, names[dataIdx]);
                    }else {
                        TextColored(colors[dataIdx], "%i: %8.4g | %s", v_idx, v0, names[dataIdx]);
                    }
                }
            }
            ImGui::EndTooltip();
    ...
    void PlotMultiLines(
            ...
            ImVec2 graph_size, int stride, float max)
    {
        PlotMultiEx(ImGuiPlotType_Lines, label, num_datas, names, colors, getter, datas, values_count, scale_min, scale_max, graph_size, stride, max);
    }
    ...
    void PlotMultiHistograms(
            ...
            ImVec2 graph_size, int stride, float max)
    {
        PlotMultiEx(ImGuiPlotType_Histogram, label, num_hists, names, colors, getter, datas, values_count, scale_min, scale_max, graph_size, stride, max);
    }
}
