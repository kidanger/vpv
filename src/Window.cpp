...
void Window::display()
{
...

    {
        ...
        ImVec2 rmin(wpos.x + ImGui::GetWindowWidth() - 84, wpos.y);
        ImVec2 rmax(wpos.x + ImGui::GetWindowWidth() - 44, wpos.y + 20);
        if (ImGui::IsMouseHoveringRect(rmin, rmax, /*clip=*/false)) {
            if (index > 0) {
    ...
    if (ImGui::IsWindowFocused()) {
        ...
        if (gSelectionShown) {
            for (const auto& win : gWindows) {
                ...
                win->histogram->request(img, rect);
            }
        }
...
std::shared_ptr<Sequence> Window::getCurrentSequence() const
{
    if (sequences.empty())
        return nullptr;
